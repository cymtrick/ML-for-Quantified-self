...
dataset_test = pd.read_csv('../processed_x_test.csv')
dataset_train = pd.read_csv('../processed_X_train.csv')
# Set up the file names and locations.
# DATA_PATH = Path('../')
# DATASET_FNAME = sys.argv[1] if len(sys.argv) > 1 else 'chapter3_result_train.csv'
# RESULT_FNAME = sys.argv[2] if len(sys.argv) > 2 else 'chapter3_result_final.csv'
# ORIG_DATASET_FNAME = sys.argv[3] if len(sys.argv) > 3 else 'processed_X_train.csv'
...
selected_predictor_cols = [c for c in dataset_train.columns if (not ('label' in c)) and (not (c == 'hr_watch_rate'))]
pc_values = PCA.determine_pc_explained_variance(dataset_train, selected_predictor_cols)
...
dataset = PCA.apply_pca(copy.deepcopy(dataset_train), selected_predictor_cols, n_pcs)

#And we visualize the result of the PC's

DataViz.plot_dataset(dataset_train, ['pca_', 'label'], ['like', 'like'], ['line', 'points'])

# And the overall final dataset:

DataViz.plot_dataset(dataset_train, ['acc_', 'gyr_', 'hr_watch_rate', 'light_phone_lux', 'mag_', 'press_phone_', 'pca_', 'label'],
                     ['like', 'like', 'like', 'like', 'like', 'like', 'like','like', 'like'],
                     ['line', 'line', 'line', 'line', 'line', 'line', 'line', 'points', 'points'])

# Store the outcome.

dataset.to_csv("chapter_3_train.csv")